...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const exponentiation = (x2 - x1) ** 2 + (y2 - y1) ** 2;
  const result = Math.sqrt(`${exponentiation}`);
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cos = vectors / (moduleA * moduleB);

  const result = Math.acos(cos);
